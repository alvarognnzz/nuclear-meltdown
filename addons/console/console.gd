...
func _input(event : InputEvent) -> void:
	if (event is InputEventKey):
		if (event.get_physical_keycode_with_modifiers() == KEY_F8):
			if (event.pressed):
				toggle_console()
...
func toggle_console() -> void:
	get_tree().get_first_node_in_group("character").movement_enabled = not get_tree().get_first_node_in_group("character").movement_enabled
	get_tree().get_first_node_in_group("character").velocity = Vector3.ZERO
